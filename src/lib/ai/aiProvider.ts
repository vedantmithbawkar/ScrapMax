...
export class GeminiAIProvider implements AIProvider {
  async interpretCommand(input: string, context: CommandContext): Promise<CommandIntent | null> {
    try {
      ...
      if (!response.ok) {
        console.warn('GeminiAIProvider API returned non-OK status:', response.status);
        return null;
      }
      ...
      console.error('GeminiAIProvider error parsing command:', err);
      return null; // Fallback to null on error/malformed JSON
    }
  }
}
